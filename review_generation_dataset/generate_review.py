...
def read_single_json(file_):
    try:
        reviews = [json.loads(line) for line in file_]
    except:
        return [], []
    # print("rev")
    # print(reviews)
...
def write_reivew_to_file(path, reviews, scores, count):
    file_path = os.path.abspath(os.path.join(path, "%06d.txt" % (count // 1000)))
    write_file = codecs.open(file_path, "a", "utf-8")

    for i, review in enumerate(reviews):

        dict = {"review": str(review), "score": str(scores[i])}
...
file_path = "review_generation_dataset/data"
...
for i, file in enumerate(valid_filename):
    file_ = codecs.open(file_path + "/" + file, "r", "utf-8", errors="replace")
    reviews, scores = read_single_json(file_)
    write_reivew_to_file("review_generation_dataset/valid", reviews, scores, i)
...
for i, file in enumerate(test_filename):
    ...
    write_reivew_to_file("review_generation_dataset/test", reviews, scores, i)
